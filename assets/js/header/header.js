  ...
  if(token ===null || token === undefined || token === NaN || token === ''){
    ...
    .then(account => {
      ...
      if(listNotify==[] || listNotify == null || listNotify == undefined){
        ...
        console.log(111);
        if(quantityNotify == 0){
        document.querySelector('.header__notify-notice').style.display = 'none'
        }else if(quantityNotify > 9){
        document.querySelector('.header__notify-notice').innerHTML = '9+';
        }else{
        document.querySelector('.header__notify-notice').innerHTML = quantityNotify;
        }}   
      })
...
$("#addformModal").validate({
  ...
  onclick: false,
  rules: {
      "nameFoodModal": { // name element not id
          required: true
      },
      "categoryModal": {
          required: true
...
document.getElementById("upload_image_foodModal").addEventListener("click", function () {
	myWidgetFood.open();
},
	false
);
